        ...
        for case, c_results in t_results.items():
            ...
            for m_i, m in enumerate(sorted_models):
                        ...
                        if pm == m and pc[0] == c:
                            for other_i, other in enumerate(conds[1:]):
                                if pc[1] == other:
                                    ...
                                    if pp < 0.1:
                                        ax.vlines(
                                                  ymin=start-(other_i*0.03), 
                                                  ymax=start-(other_i*0.03)-0.01, 
                ...
                if m_i in [2, 4]:
                    ...
                    counter += 1
            '''
            ax.set_ylim(bottom=gen_avg-(gen_std*6), top=gen_avg+(gen_std*3))
            ax.text(
                    2.,
                    0.34,
            ...
            pyplot.ylabel('Residual log10(RT)', fontsize=23)
            pyplot.xticks(
                          #[x[0] for x in xticks], 
                          corrections,
                          #[x[1] for x in xticks],
