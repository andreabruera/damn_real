...
### reading simlx in german
sims = dict()
with open(os.path.join('data', 'MSimLex999_German.txt')) as i:
    for l_i, l in enumerate(i):
        if l_i == 0:
            continue
        line = l.strip().split(',')
        w_one = line[0].lower()
        w_two = line[1].lower()
        if w_one in vocab.keys() and vocab[w_one]!=0 and vocab[w_one] in coocs.keys() and w_one in freqs.keys():
            if w_one not in uttered_words:
                uttered_words.append(w_one)
        else:
            continue
        if w_two in vocab.keys() and vocab[w_two]!=0 and vocab[w_two] in coocs.keys() and w_two in freqs.keys():
            if w_two not in uttered_words:
                uttered_words.append(w_two)
        else:
            continue
        sim = float(line[-1])
        sims[tuple(sorted([w_one, w_two]))] = sim
...
### adding missing keys to the ratings
to_be_added = [w for w in uttered_words if w not in pruned_ratings.keys()]
...
for orig_percent in [
    ...
    percent_damage = orig_percent*0.02
    ...
    if orig_percent == 0:
        ...
        undam_trans_pmi_vecs = build_ppmi_vecs(coocs, vocab, mtrx_ctx_words, mtrx_ctx_words, smoothing=True)
        ...
        for dim in sem_dims:
            ...
            new_sem_dims = set([var for k,v in additional_ratings.items() for var in v.keys()])
            if dim not in new_sem_dims:
                training_items = [(undam_trans_pmi_vecs[k], pruned_ratings[k][dim]) for k in pruned_ratings.keys() if k not in to_be_added]
                test_items = [(undam_trans_pmi_vecs[k], k) for k in to_be_added]
                ridge = sklearn.linear_model.RidgeCV(alphas=(0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000))
                ridge.fit([v[0] for v in training_items], [[v[1]] for v in training_items])
                preds = ridge.predict([v[0] for v in test_items])
                for k, pred in zip(to_be_added, preds):
                    if k not in additional_ratings.keys():
                        additional_ratings[k] = dict()
                        pruned_ratings[k] = dict()
                    additional_ratings[k][dim] = float(pred)
                    pruned_ratings[k][dim] = float(pred)
    ...
    real = list()
    pred = list()
    for ws, sim in sims.items():
        real.append(sim)
        pred.append(1-scipy.spatial.distance.cosine(
                            damaged_ppmi[orig_percent][ws[0]], 
                            damaged_ppmi[orig_percent][ws[1]]
                            )
                            )
    print(orig_percent)
    print(percent)
    print(scipy.stats.pearsonr(real, pred))
